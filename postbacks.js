...
exports.contact_broker = (sender, values) => {
    messenger.send({text: "Here is the recruiter information for this job"}, sender);
    messenger.send(formatter.formatBroker(), sender);
};

exports.confirm_visit = (sender, values) => {
    messenger.send({text: `OK, your interview is confirmed for ${values[2]}. ${values[1]}.`}, sender);
};
...
exports.contact_me = (sender, values) => {
    ...
    messenger.getUserInfo(sender).then(response => {
        salesforce.createCase(propertyId, response.first_name + " " + response.first_name, sender).then(() => {
            messenger.send({text: `Thanks for your interest, ${response.first_name}. I asked a recruiter to contact you asap.`}, sender);
        });
    });
