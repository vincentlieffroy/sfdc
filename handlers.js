...
exports.searchHouse = (sender) => {
    messenger.send({text: `OK, looking for jobs around you...`}, sender);
...
exports.searchHouse_City = (sender, values) => {
    messenger.send({text: `OK, looking for jobs in ${values[1]}`}, sender);
...
exports.searchHouse_Bedrooms_City_Range = (sender, values) => {
    messenger.send({text: `OK, looking for ${values[1]} year-experience jobs in ${values[2]} with salary between ${values[3]} and ${values[4]}`}, sender);
...
exports.searchHouse_Bedrooms_City = (sender, values) => {
    messenger.send({text: `OK, looking for ${values[1]} year-experience jobs in ${values[2]}`}, sender);
    salesforce.findProperties({bedrooms: values[1], city: values[2]}).then(properties => {
        messenger.send(formatter.formatProperties(properties), sender);
    });
};
...
exports.searchHouse_Bedrooms = (sender, values) => {
    messenger.send({text: `OK, looking for ${values[1]} year-experience jobs`}, sender);
...
exports.searchHouse_Range = (sender, values) => {
    messenger.send({text: `OK, looking for jobs with salary between ${values[1]} and ${values[2]}`}, sender);
...
exports.priceChanges = (sender, values) => {
    messenger.send({text: `OK, looking for recent salary changes...`}, sender);
